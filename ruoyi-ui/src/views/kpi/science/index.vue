<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="科技成果名称" prop="scienceName">
        <el-input
          v-model="queryParams.scienceName"
          placeholder="请输入科技成果名称"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="科技成果合同编号" prop="scienceNumber">
        <el-input
          v-model="queryParams.scienceNumber"
          placeholder="请输入科技成果合同编号"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['kpi:science:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="scienceList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="科技成果id" align="center" prop="scienceId" />
      ...
      <el-table-column label="科技成果名称" align="center" prop="scienceName" />
      <el-table-column label="科技成果合同编号" align="center" prop="scienceNumber" />
      <el-table-column label="发表日期" align="center" prop="publishTime" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.publishTime, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="金额" align="center" prop="scienceMoney" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="科技成果名称" prop="scienceName">
          <el-input v-model="form.scienceName" placeholder="请输入科技成果名称" disabled/>
        </el-form-item>
        <el-form-item label="科技成果合同编号" prop="scienceNumber">
          <el-input v-model="form.scienceNumber" placeholder="请输入科技成果合同编号" disabled/>
        </el-form-item>
        ...
        <el-form-item label="金额" prop="scienceMoney">
          <el-input v-model="form.scienceMoney" placeholder="请输入金额" disabled/>
        </el-form-item>
...
<script>
import { listScience, getScience, delScience, addScience, updateScience } from "@/api/kpi/science";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        scienceName: [
          { required: true, message: "科技成果名称不能为空", trigger: "blur" }
        ],
        scienceNumber: [
          { required: true, message: "科技成果合同编号不能为空", trigger: "blur" }
        ],
        publishTime: [
  ...
  methods: {
    ...
    getMagnitudeList(){
      listMagnitude({ pageNum: 1,
        pageSize: 1000,typeId:10}).then(response => {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.scienceId)
    ...
    handleUpdate(row) {
      this.reset();
      const scienceId = row.scienceId || this.ids
      getScience(scienceId).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.scienceId != null) {
            updateScience(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addScience(this.form).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const scienceIds = row.scienceId || this.ids;
      this.$modal.confirm('是否确认删除科技成果编号为"' + scienceIds + '"的数据项？').then(function() {
        return delScience(scienceIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('kpi/science/export', {
        ...this.queryParams
      }, `science_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
